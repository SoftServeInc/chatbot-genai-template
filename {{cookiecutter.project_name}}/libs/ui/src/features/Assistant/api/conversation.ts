...
import { useBotExplorerStore } from '../../BotExplorer/state/store';
...
export async function streamMessage(
  ...
  const { logRequest, logResponse } = useBotExplorerStore.getState();
  ...
  const botExplorerConfig = localStorage.getItem('bot_explorer.config');
  if (botExplorerConfig) {
    const { baseURL, accessToken } = JSON.parse(botExplorerConfig);
  ...
  logRequest({
    ...
    url: `/chats/${chatId}/converse-stream`,
    method,
    data: { data: [{ type: 'text', content: message_content }] },
  });
